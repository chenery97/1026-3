...
// 用户名查询的post请求路由
router.post('/userSearch', async (request, response) => {
  const re = await personModel.findOne(request.body)
  if (!re) {
    response.send('{"code":10000, "msg":"用户名可用"}')
  } else {
    response.send('{"code":11000, "msg":"用户名已被使用"}')
  }
})
// 登录页面的post请求路由
